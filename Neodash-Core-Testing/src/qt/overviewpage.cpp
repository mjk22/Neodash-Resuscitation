...
public:
    TxViewDelegate(const PlatformStyle *platformStyle):
        QAbstractItemDelegate(), unit(BitcoinUnits::NDASH),
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    if(model && model->getOptionsModel())
    {
        ...
        connect(model->getOptionsModel(), SIGNAL(anonymizeNeodashAmountChanged()), this, SLOT(updatePrivateSendProgress()));
    ...
    // update the display unit, to not use the default ("NDASH")
    updateDisplayUnit();
}
...
void OverviewPage::updatePrivateSendProgress()
{
    ...
    QString strAnonymizeNeodashAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeNeodashAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeNeodashAmount = strAnonymizeNeodashAmount.remove(strAnonymizeNeodashAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeNeodashAmount + " / " + tr("%n Rounds", "", nPrivateSendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeNeodashAmount*COIN) nMaxToAnonymize = nAnonymizeNeodashAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeNeodashAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeNeodashAmount));
        strAnonymizeNeodashAmount = strAnonymizeNeodashAmount.remove(strAnonymizeNeodashAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeNeodashAmount + " / " + tr("%n Rounds", "", nPrivateSendRounds);
    } else {
