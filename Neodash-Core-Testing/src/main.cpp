...
#if defined(NDEBUG)
# error "Neodash cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams)
{
    ...
    if(nPrevHeight > 5 || Params().NetworkIDString() != CBaseChainParams::MAIN) dDiff = ConvertBitsToDouble(nPrevBits);

    CAmount nSubsidy = 0;
    if(nPrevHeight >= 6) {
        if((nPrevHeight >= 17 && dDiff > 75) || nPrevHeight >= 24) { // GPU/ASIC difficulty calc
            // 2222222/(((x+2600)/9)^2)
            nSubsidy = (2222222.0 / (pow((dDiff+2600.0)/9.0,2.0)));
            if (nSubsidy > 5) nSubsidy = 5;
            if (nSubsidy < 2.5) nSubsidy = 2.5;
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /* 
        Neodash Related Inventory Messages
