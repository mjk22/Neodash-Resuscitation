...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help"))
    {
        ...
        std::string strUsage = _("Neodash Core neodash-tx utility version") + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  neodash-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded neodash transaction") + "\n" +
              "  neodash-tx [options] -create [commands]   " + _("Create hex-encoded neodash transaction") + "\n" +
              "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded neodash transaction
