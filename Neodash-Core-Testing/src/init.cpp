...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, mempoolrej, net, proxy, prune, http, libevent, tor, zmq, "
                             "neodash (or specifically: privatesend, instantsend, masternode, keepass, mnpayments, mngovernance)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Neodash specific functionality (Masternodes, PrivateSend, InstantSend, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:7135"));
    ...
    strUsage += HelpMessageOpt("-anonymizeneodashamount=<n>", strprintf(_("Keep N NDASH anonymized (default: %u)"), nAnonymizeNeodashAmount));
    ...
    strUsage += HelpMessageOpt("-instantsenddepth=<n>", strprintf(_("Show N confirmations for a successfully locked transaction (0-7135, default: %u)"), nInstantSendDepth));
...
std::string LicenseInfo()
{
    ...
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2014-%i The Neodash Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Neodash is running in a usable environment with all
...
void InitParameterInteraction()
{
    ...
    if (GetArg("-liquidityprovider", 0) > 0) {
        ...
        mapArgs["-anonymizeneodashamount"] = "999999";
        LogPrintf("AppInit2 : parameter interaction: -liquidityprovider=%d -> setting -anonymizeneodashamount=999999\n", nLiqProvTmp);
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Neodash Core is shutting down."));
...
#endif
    // Make sure only a single Neodash process is using the data directory.
    ...
    try {
        ...
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Neodash Core is probably already running."), strDataDir));
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Neodash Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Neodash Core to complete") << "\n";
    ...
    nAnonymizeNeodashAmount = GetArg("-anonymizeneodashamount", nAnonymizeNeodashAmount);
    nAnonymizeNeodashAmount = std::min(std::max(nAnonymizeNeodashAmount, 2), 999999);
    ...
    LogPrintf("Anonymize Neodash Amount %d\n", nAnonymizeNeodashAmount);
    ...
    // ********************************************************* Step 11c: update block tip in Neodash modules
    ...
    // ********************************************************* Step 11d: start neodash-privatesend thread
