# mininode.py - Neodash P2P network half-a-node
#
...
#
# NodeConn: an object which manages p2p connectivity to a neodash node
...
def neodashhash(s):
    return neodash_hash.getPoWHash(s)
...
# Objects that map to neodashd objects, which can be serialized/deserialized
...
class CBlockHeader(object):
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(neodashhash(r))
            self.hash = neodashhash(r)[::-1].encode('hex_codec')
...
class msg_headers(object):
    ...
    def deserialize(self, f):
        # comment in neodashd indicates these should be deserialized as blocks
...
class NodeConn(asyncore.dispatcher):
    ...
    def __init__(self, dstaddr, dstport, rpc, callback, net="regtest", services=1):
        ...
        print 'MiniNode: Connecting to Neodash Node IP # ' + dstaddr + ':' \
            + str(dstport)
